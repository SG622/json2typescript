...
export abstract class JsonConvert {
  ...
  private static deserializeObject_loopProperty(classInstance: any, propertyKey: string, json: Object): void {
    ...
    let mapping = __jsonconvert__mapping__[classInstance.constructor.name];
  ...
  private static deserializeObject_mapProperty(expectedType: any, jsonValue: any): any {
    ...
    if (expectedType instanceof Array === false && jsonValue instanceof Array === false) {
      ...
      if (expectedType.prototype.constructor && expectedType.prototype.constructor.name && __jsonconvert__mapping__[expectedType.prototype.constructor.name]) {
...
/**
 * Merges the mapping info from the given class
 * @param mi The target mapping info.
 * @param className The given class name.
 * @returns {any[]} The target mapping info.
 */
function mergeMappingInfo(mi: any[], className: string) {
  if(className && __jsonconvert__mapping__[className]) {
    mi = Object.assign(mi, __jsonconvert__mapping__[className]);
  }
  return mi;
}
...
export function JsonObject(target: any) {
  if(target.prototype.constructor.name && typeof(__jsonconvert__mapping__[target.prototype.constructor.name]) === "undefined") {
    __jsonconvert__mapping__[target.prototype.constructor.name] = mergeMappingInfo([], target.prototype.__proto__.constructor.name);
  }
}
...
export function JsonProperty(jsonKey: string, expectedType?: any, isOptional?: boolean): any {
  ...
  return function (target: any, key: string): void {
    ...
    if (typeof(__jsonconvert__mapping__[target.constructor.name]) === "undefined") {
      __jsonconvert__mapping__[target.constructor.name] = mergeMappingInfo([], target.__proto__.constructor.name);
    }
    ...
    __jsonconvert__mapping__[target.constructor.name][key] = {
      "jsonKey": jsonKey,
      "type": expectedType,
      "optional": isOptional
    };
